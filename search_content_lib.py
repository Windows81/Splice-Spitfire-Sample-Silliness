...
def process_items(data: dict[str, dict[str, Any]], non_nested_keys: set[str]):
    # Note that `data` is mutable.

    if len(data) == 0:
        return

    # Pass 1: generates nested dicts in each of `data`, which will be populated by the next pass.
    replacement_keys = defaultdict(list)
    for iden, item in data.items():
        for k, v in list(item.items()):
            non_nested_keys.add(iden)
            ...
            if suffix == 'id' and isinstance(v, str):
                replacement_keys[v].append((item, f'{base}_item'))

            # For example, when a key is `"item_ids"`
            if suffix == 'ids' and isinstance(v, list):
                rep_dict = item[f'{base}_items'] = {}
                for i in v:
                    replacement_keys[i].append((rep_dict, i))

    # Pass 2: populates nested dicts.
    replacement_key_keys = set(replacement_keys).difference(non_nested_keys)
    replacement = get_info(list(replacement_key_keys))
    process_items(replacement, non_nested_keys)
    for k, item in replacement.items():
        for (dict_to_add, key_to_add) in replacement_keys[k]:
...
def main():
    ...
    json.dump(data, open('content_lib.json', 'w'), indent='\t')
